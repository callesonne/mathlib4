...
`(postcompose Cᵒᵖ A B).obj F ⋙ presheafToSheaf J B`.
...
class PreservesSheafification : Prop where
  le : J.W ≤ J.W.inverseImage ((postcompose Cᵒᵖ A B).obj F)
...
lemma W_isInvertedBy_postcompose_presheafToSheaf :
    J.W.IsInvertedBy (((postcompose Cᵒᵖ A B).obj F) ⋙ presheafToSheaf J B) := by
  intro P₁ P₂ f hf
  dsimp
  rw [← W_iff]
...
noncomputable abbrev Sheaf.composeAndSheafify : Sheaf J A ⥤ Sheaf J B :=
  sheafToPresheaf J A ⋙ (postcompose _ _ _).obj F ⋙ presheafToSheaf J B

variable [HasWeakSheafify J A]

/-- The canonical natural transformation from
`(postcompose Cᵒᵖ A B).obj F ⋙ presheafToSheaf J B` to
...
noncomputable def toPresheafToSheafCompComposeAndSheafify :
  ...
  whiskerRight (sheafificationAdjunction J A).unit
    ((postcompose _ _ _).obj F ⋙ presheafToSheaf J B)
...
and `(postcompose Cᵒᵖ A B).obj F ⋙ presheafToSheaf J B` when `F : A ⥤ B`
...
noncomputable instance : Localization.Lifting (presheafToSheaf J A) J.W
    ((postcompose Cᵒᵖ A B).obj F ⋙ presheafToSheaf J B) (Sheaf.composeAndSheafify J F) :=
...
`(postcompose Cᵒᵖ A B).obj F ⋙ G₂ ⟶ G₁ ⋙ sheafCompose J F`
...
def sheafComposeNatTrans :
    (postcompose Cᵒᵖ A B).obj F ⋙ G₂ ⟶ G₁ ⋙ sheafCompose J F where
